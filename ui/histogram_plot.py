...
class HistogramViewer(BaseWidget):
  ...
  def __init__(self, parentFrame, image_container):
    ...
    lowSlider = ttk.Scale(frame, variable=self.low, from_=0, to=255, length=300, orient='horizontal', command=self.slider_changed)
    highSlider = ttk.Scale(frame, variable=self.high, from_=0, to=255, length=300, orient='horizontal', command=self.slider_changed)
    lowSlider.place(x=0, y=0)
    highSlider.place(x=0, y=20)
